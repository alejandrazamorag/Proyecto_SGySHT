...
<head>
  ...
  <script src="./../../../css/select2/js/select2.js"></script>
...
<script language="javascript" type="text/javascript">
  ...
  miCampoTexto = document.getElementById("NombreAlumno").value;
        ...
        if (miCampoTexto.length == 0 || /^\s+$/.test(miCampoTexto)) {
          alert('Verifica el campo Nombre(s) del Alumno esta vacio!');
          return false;
        }

        miCampoTexto = document.getElementById("ApellidoP").value;
        if (miCampoTexto.length == 0 || /^\s+$/.test(miCampoTexto)) {
          alert('Verifica el campo Apellido Paterno del Alumno esta vacio!');
          return false;
        }

        miCampoTexto = document.getElementById("ApellidoM").value;
        if (miCampoTexto.length == 0 || /^\s+$/.test(miCampoTexto)) {
          alert('Verifica el campo Apellido Materno del Alumno esta vacio!');
          return false;
        }

        miCampoTexto = document.getElementById("CURP").value;
        if (miCampoTexto.length == 0 || /^\s+$/.test(miCampoTexto)) {
          alert('Verifica el campo CURP del Alumno esta vacio!');
          return false;
        }

        miCampoTexto = document.getElementById("CURP").value;
        if (miCampoTexto.length == 0 || !/^[a-zA-Z0-9][a-zA-Z0-9]*$/.test(miCampoTexto)) {
          alert('Verifica CURP incorrecta');
          return false;
        }
        ...
        if(!isNaN(txtFecha)){
          alert('Selecciona o introduzca elegir una fecha');
          return false;
        }
...
if (miCampoTexto.length == 0 || /^\s+$/.test(miCampoTexto)) {
  alert('Verifica el campo Estado del Domicilio esta vacio!');
  return false;
}
...
<body>
 ...
 <div class="container">
  ...
  <br><center><h2> Modificacion Datos Basicos del alumno... </h2></center>
  <!--Poner titulo o nombre -->
  ...
  <div class="row">
    ...
    <div class="col-sm-9">
     ...
     <?php if((!isset($_POST['NombreAlumno']))&&(!isset($_POST['ApellidoP']))&&(!isset($_POST['ApellidoM']))&&(!isset($_POST['CURP']))&&(!isset($_POST['FechaNacimiento']))&&(!isset($_POST['Correo']))&&(!isset($_POST['Sexo']))&&(!isset($_POST['TelefonoEmergencia']))&&(!isset($_POST['LugarNacimiento']))&&(!isset($_POST['NombreAlumno']))){?>
      <form action="ModificacionDatosBasico.php" method="POST"  onsubmit="return validar()">
        <?php 
        include_once "../../clases/MySQLConector.php";

        $Mysql = new MySQLConector();
        $Mysql->Conectar();
        $Consulta = "SELECT alumno.Nombre, alumno.ApellidoP, alumno.ApellidoM, alumno.CURP, alumno.FechaNac, alumno.Correo, alumno.Sexo, alumno.TelefonoEmergencia, alumno.LugarNacimiento_idLugarNacimiento AS idLugarNacimiento, alumno.Grupos_idGrupos AS idGrupos, alumno.NC, alumno.SS, alumno.Estatus, domicilio.CP, domicilio.Calle, domicilio.Numero, domicilio.Colonia, domicilio.Municipio, domicilio.Localidad, domicilio.Estado, domicilio.TelefonoCasa, datosescolares.Secundaria_idSecundaria AS idSecundaria, datosescolares.PromedioSecu, datosmedicos.Enfermedad, datosmedicos.Tratamiento, datosmedicos.Tabaquismo, datosmedicos.Drogadiccion, datosmedicos.Drogadiccion, datosmedicos.Alcoholismo from alumno, domicilio, datosescolares, datosmedicos WHERE alumno.idAlumno = '".$_SESSION['IdAlumnoCapturista']."' AND alumno.Domicilio_idDomicilio = domicilio.idDomicilio AND datosescolares.Alumno_idAlumno = '".$_SESSION['IdAlumnoCapturista']."' AND datosmedicos.Alumno_idAlumno = '".$_SESSION['IdAlumnoCapturista']."' ";
        ...
        while ($fila = $Resultado->fetch_assoc()) { 
          ...
          <h4>Datos Alumno:</h4>
          <b><i>Nombre(s):</i></b>
          <?php echo "<input type=\"text\" name=\"NombreAlumno\" class=\"form-control\" id=\"NombreAlumno\" placeholder=\"Nombre\" value=\"".$fila['Nombre']."\"  onkeypress=\"return soloLetras(event);\" maxlength=\"25\" >"; ?>
          <b><i>Apellido Paterno:</i></b>
          <?php echo "<input type=\"text\" name=\"ApellidoP\" class=\"form-control\" id=\"ApellidoP\" placeholder=\"Apellido Paterno\" value=\"".$fila['ApellidoP']."\" onkeypress=\"return soloLetras(event);\" maxlength=\"25\" >";?>
          <b><i>Apellido Materno:</i></b>
          <?php echo "<input type=\"text\" name=\"ApellidoM\" class=\"form-control\" id=\"ApellidoM\" placeholder=\"Apellido Materno\" value=\"".$fila['ApellidoM']."\" onkeypress=\"return soloLetras(event);\" maxlength=\"25\" >";?>
          <b><i>CURP:</i></b>
          <?php echo"<input type=\"text\" name=\"CURP\" class=\"form-control\" id=\"CURP\" placeholder=\"CURP\" value=\"".$fila['CURP']."\" maxlength=\"18\" onKeyUp=\"this.value = this.value.toUpperCase();\" onkeypress=\"return NumerosLetrasSinCaracteres(event);\" >";?>
          <b><i>Fecha Nacimiento:</i></b>
          <?php echo"<input type=\"date\" name=\"FechaNacimiento\" class=\"form-control\" id=\"FechaNacimiento\" value=\"".$fila['FechaNac']."\" >";?>
          <b><i>Correo:</i></b>
          <?php echo"<input type=\"email\" name=\"Correo\" class=\"form-control\" id=\"Correo\" placeholder=\"Correo\" value=\"".$fila['Correo']."\" onkeypress=\"return  NumerosLetrasCorreo(event);\" maxlength=\"50\">";?>
          <b><i>Sexo:</i></b>
          <select name="Sexo" class="m-1 custom-select">
            <option value="F" <?php if($fila['Sexo'] == 'F') echo "Selected"; ?>>F</option>
            <option value="M" <?php if($fila['Sexo'] == 'M') echo "Selected"; ?>>M</option>
          </select>
          <b><i>Telefono Emergencia:</i></b>
          <?php echo"<input type=\"text\" name=\"TelefonoEmergencia\" class=\"form-control\" id=\"TelefonoEmergencia\" onKeyPress=\"return soloNumeros(event)\" placeholder=\"Telefono de Emergencia\" value=\"".$fila['TelefonoEmergencia']."\"  onKeyPress=\"return soloNumeros(event)\" maxlength=\"10\" >";?>
          <b><i>Numero Control:</i></b>
          <?php echo"<input type=\"text\" name=\"NC\" class=\"form-control\" id=\"NC\" placeholder=\"Numero Control\" onKeyPress=\"return soloNumeros(event)\" maxlength=\"14\" value=\"".$fila['NC']."\" >";?>
          <b><i>Seguro Social:</i></b>
          <?php echo"<input type=\"text\" name=\"SS\" class=\"form-control\" id=\"SS\" placeholder=\"Seguro Social\" onKeyPress=\"return soloNumeros(event)\" maxlength=\"11\" value=\"".$fila['SS']."\">";?>
          <br>
          <h4>Grado y Grupo:</h4>
          <?php
          include_once "../../clases/MySQLConector.php";
          ...
          <select name="GradoGrupo" id="GradoGrupo" class="form-control input-sm"  > 
           <?php
           while ($filaGG = $Resultado->fetch_assoc()) { 
             echo " <option value=\"{$filaGG['idGrupos']}\""; if($fila['idGrupos'] == $filaGG['idGrupos']) echo "selected"; echo">{$filaGG['Grado']}-{$filaGG['Grupo']}, {$filaGG['Nombre']}</option>";
           }
           ?>
         </select>
         <br>
         ...
         <select name="LugarNacimiento" id="LugarNacimiento" class="m-1 custom-select"> 
          ...
          while ($fila2 = $Resultado->fetch_assoc()) { 
            ?>
            <?php 
            echo " <option value=\"{$fila2['idLugarNacimiento']}\""; if($fila['idLugarNacimiento'] == $fila2['idLugarNacimiento']) echo "selected"; echo "> {$fila2['Municipio']}, {$fila2['Localidad']}, {$fila2['Estado']}, {$fila2['Pais']}</option>";
          }
          ?>
        </select>
        <div class="radio">  
          ...
          <b>Estado</b>
          <input type="text" name="Estadol" id="Estadol" class="form-control input-sm" placeholder="Estado" onkeypress="return soloLetras(event);" disabled maxlength="30">
          <b>Localidad</b>
        ...
        <h4>Domicilio</h4>
        <b>CP</b>
        <?php echo" <input type=\"text\" name=\"CPd\" id=\"CPd\" class=\"form-control input-sm\" placeholder=\"CP\" value=\"".$fila['CP']."\" onKeyPress=\"return soloNumeros(event)\" maxlength=\"5\">"?>
        <b>Calle</b>
        <?php echo"<input type=\"text\" name=\"Called\" id=\"Called\" class=\"form-control input-sm\" placeholder=\"Calle\" value=\"".$fila['Calle']."\" onkeypress=\"return NumerosLetras(event);\" maxlength=\"30\">"?>
        <b>Numero</b>
        <?php echo"<input type=\"text\" name=\"Numerod\" id=\"Numerod\" class=\"form-control input-sm\" placeholder=\"Numero\" value=\"".$fila['Numero']."\" onkeypress=\"return NumerosLetras(event);\" maxlength=\"20\">"?>
        <b>Colonia</b>
        <?php echo"<input type=\"text\" name=\"Coloniad\" id=\"Coloniad\" class=\"form-control input-sm\" placeholder=\"Colonia\" value=\"".$fila['Colonia']."\"  onkeypress=\"return NumerosLetras(event);\" maxlength=\"50\" >"?>
        <b>Municipio</b>
        <?php echo"<input type=\"text\" name=\"Municipiod\" id=\"Municipiod\" class=\"form-control input-sm\" placeholder=\"Municipio\" value=\"".$fila['Municipio']."\" >"?>
        <b>Localidad</b>
        <?php echo"<input type=\"text\" name=\"Localidadd\" id=\"Localidadd\" class=\"form-control input-sm\" placeholder=\"Localidad\" value=\"".$fila['Localidad']."\" onkeypress=\"return NumerosLetras(event);\" maxlength=\"50\" >"?>
        <b>Estado</b>
        <?php echo"<input type=\"text\" name=\"Estadod\" id=\"Estadod\" class=\"form-control input-sm\" placeholder=\"Estado\" value=\"".$fila['Estado']."\" onkeypress=\"return soloLetras(event);\" maxlength=\"50\" >"?>
        <b>Tel. Casa</b>
        <?php echo"<input type=\"text\" name=\"TelCasad\" id=\"TelCasad\" onKeyPress=\"return soloNumeros(event)\" class=\"form-control input-sm\" placeholder=\"Tel. Casa\" value=\"".$fila['TelefonoCasa']."\"  onKeyPress=\"return soloNumeros(event)\" maxlength=\"10\">"?> 
        <br>
        
        <h4>Escuela Secundaria de Procedencia</h4>
        <b><i>Secundaria:</i></b>
        <?php
        include_once "../../clases/MySQLConector.php";
        ...
        $Resultado = $Mysql->Consulta($Consulta);
        ?>
        <select name="Secundaria" id="Secundaria" class="m-1 custom-select"> 
          <?php
          while ($fila3 = $Resultado->fetch_assoc()) { 
            ?>
            <?php 
            echo "<option value=\"{$fila3['idSecundaria']}\"";if($fila['idSecundaria'] == $fila3['idSecundaria']){ echo "Selected"; }  echo">{$fila3['Nombre']}, {$fila3['Municipio']}, {$fila3['Estado']}, {$fila3['Pais']}</option>";
          }
          ?>
        </select> 
        <div class="radio">  
          ...
          <b>Estado:</b><input type="text" name="Estado" id="Estado" disabled="disabled" class="form-control" placeholder="Estado" onkeypress="return soloLetras(event);" onkeypress="return soloLetras(event);" maxlength="50"/>
          <b>Pais:</b><input type="text" name="Pais" id="Pais" disabled="disabled" class="form-control" placeholder="Pais" onkeypress="return soloLetras(event);" onkeypress="return soloLetras(event);" maxlength="20"/>
        </div>
        <b><i>Promedio:</i></b>
        <?php echo"<input type=\"text\" name=\"Promedio\" class=\"form-control\" id=\"Promedio\" placeholder=\"Promedio\" value=\"".$fila['PromedioSecu']."\" onkeypress=\"return NumeroDecimal(event, this);\" >"?>
        ...
        <?php echo"<input type=\"text\" name=\"Enfermedad\" class=\"form-control\" id=\"Enfermedad\" placeholder=\"Escribe tu respuesta\" value=\"".$fila['Enfermedad']."\">"?>
        <b><i>Actualmente ¿recibes algún tratamiento médico psicólogico? ¿Lo has recibido alguna vez?</i></b>
        <?php 
        $Opciones= explode(",",  $fila['Tratamiento']);
            //print_r($Opciones);
        ...
        if (count($Opciones) >= 2) {
         ...
         <div class="form-check-inline">
          Si <input class="m-2" type="radio" value="O" name="Tratamiento" onclick="TipoTratamiento.disabled = false" <?php if (in_array("Si", $Opciones)) { echo "checked"; } ?> /></label>
          No <input class="m-2" type="radio" value="F" name="Tratamiento" onclick="TipoTratamiento.disabled = true"  <?php if (in_array("No", $Opciones)) { echo "checked"; } ?> /></label>
        </div>
        <br>
        <b><i>¿De que tipo?</i></b>
        <?php echo"<input type=\"text\" name=\"TipoTratamiento\" class=\"form-control\" id=\"TipoTratamiento\" placeholder=\"Escribe tu respuesta\" value=\"".$Opciones[1]."\" >"?>
        <?php
        ...
        <br>
        <b><i>¿De que tipo?</i></b>
        <?php echo"<input type=\"text\" disabled name=\"TipoTratamiento\" class=\"form-control\" id=\"TipoTratamiento\" placeholder=\"Escribe tu respuesta\" >"?>
        <?php
      }
      ?>
      <br>
      <b><i>Tabaquismo:</i></b>
      Si <input class="m-2" type="radio" value="O" name="Tabaquismo" <?php if($fila['Tabaquismo'] == '1') echo "checked"; ?>/></label>
      No <input class="m-2" type="radio" value="F" name="Tabaquismo" <?php if($fila['Tabaquismo'] == '0') echo "checked"; ?>/></label>
      <b><i>Alcohol:</i></b>
      Si <input class="m-2" type="radio" value="O" name="Alcohol" <?php if($fila['Alcoholismo'] == '1') echo "checked"; ?> /></label>
      No <input class="m-2" type="radio" value="F" name="Alcohol" <?php if($fila['Alcoholismo'] == '0') echo "checked"; ?>/></label>
      <b><i>Drogas:</i></b>
      Si <input class="m-2" type="radio" value="O" name="Drogas" <?php if($fila['Drogadiccion'] == '1') echo "checked"; ?> /></label>
      No <input class="m-2" type="radio" value="F" name="Drogas" <?php if($fila['Drogadiccion'] == '0') echo "checked"; ?>/></label>
      <br>
      <br>
      <h4>Estatus:</h4>
      <select name="Estatus" id="Estatus" class="m-1 custom-select">
        <option value="1" <?php if($fila['Estatus'] == '1') echo "Selected"; ?> >Activo</option>
        <option value="0" <?php if($fila['Estatus'] == '0') echo "Selected"; ?> >Inactivo</option>
      </select>
      <br>
      <br>
    ...
    <div class="form-group">
      <div class="col-xs-8 col-xs-offset-1">
        <button type="submit" class="btn btn-success glyphicon glyphicon-plus">Actualizar Datos</button><br>
      </div>
    </div>
    <!--Fin Button-->
  </form>
  ...
  $Alumno->setidAlumno($_SESSION['IdAlumnoCapturista']);
  ...
  $HistorialAlumno->setAlumno_idAlumno($_SESSION['IdAlumnoCapturista']);
...
//--------------------------------------------------------------------------------------------------
            ...
            $SQLControlador = new SQLControlador();
            $SQLControlador->ModificarDatosBasicosAlumno($Alumno,$LugarNacimiento,$Secundaria,$DatosEscolares,$DatosMedicos, $Domicilio, $HistorialAlumno);
//--------------------------------------------------------------------------------------------------
          }
          ?>

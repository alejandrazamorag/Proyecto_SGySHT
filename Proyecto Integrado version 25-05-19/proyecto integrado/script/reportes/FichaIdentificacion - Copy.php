...
while ($fila = $Resultado->fetch_assoc()) {
	...
	$html = '
	<h1 align="center">Ficha de Identificación del tutorado</h1>
	<h4>1. DATOS PERSONALES</h4>
	
	<b><i>Nombre:</i></b> 
	<label for="lblNombre">'.$fila['Nombre'].' '.$fila['ApellidoP'].' '.$fila['ApellidoM'].'</label><br><br>
	...
	$html .='<label for="lblFechaNacimiento">'.$newDate.'</label><br><br>
	<b><i>Edad: </i></b>
	<label for="">'.calculaedad ($fila['FechaNac']).'</label><br><br>
	<b><i>Lugar de Nacimiento: </i></b>
	<label for="lblLugarNacimiento">'.$fila['Municipio'] .', '. $fila['Estado'].'</label><br><br>
	<b><i>Domicilio: </i></b>
	<label for="lblCalle">'.$fila['Calle'].' #'.$fila['Numero'].'   Col.'.$fila['Colonia'].'   CP: '.$fila['CP'].'</label><br><br>
	<b>Municipio: </b>
	<label for="lblMunicipio">'.$fila['MunicipioDomicilio'].'</label>
	<b>Localidad: </b>
	<label for="lblLocalidad">'.$fila['Localidad'].'</label>
	<b>Estado: </b>
	<label for="lblEstado">'.$fila['EstadoDomicilio'].'</label><br><br>
	<b><i>En caso de emergencia llamar a: </i></b>
	<label for="">'.$fila['TelefonoEmergencia'].'</label><br><br>
	...
	while ($fila = $Resultado->fetch_assoc()) {
		if($fila['Parentesco'] == 'Padre'){ 
			...
			<label for="NombreP">'.$fila['Nombre'].' '.$fila['ApellidoP'].' '.$fila['ApellidoM'].'</label><br><br>
			<b>Edad: </b>
			<label for="EdadP">'.calculaedad ($fila['FechaNacimiento']).'</label><br><br>
			<b>Profesion:</b>
			<label for="ProfesionP">'.$fila['Profesion'].'</label><br><br>
			...
			<label for="NombreM">'.$fila['Nombre'].' '.$fila['ApellidoP'].' '.$fila['ApellidoM'].'</label><br><br>
			<b>Edad:</b>
			<label for="EdadM">'.calculaedad ($fila['FechaNacimiento']).'</label><br><br>
			<b>Profesion</b>
			<label for="ProfesionM">'.$fila['Profesion'].'</label><br><br>
			<b>Lugar de Trabajo</b>
	...
	while ($fila = $Resultado->fetch_assoc()) {
		...
		<label>'.$fila['LugarOcupas'].'</label><br><br>
		<b>c) Otras personas que viven con tigo: </b>
		<label>'.$fila['OtrasPersonas'].'</label><br><br>
		<b>d) Actualmente vives con: </b> 
		<label>'. $fila['ActualmenteVives'].'</label><br><br>
		...
		<label>'.$fila['SituacionFamiliar'].'</label><br><br>
	...
	while ($fila = $Resultado->fetch_assoc()) {
		...
		<b> Promedio: </b><label>'.$fila['PromedioSecu'].'</label><br><br>
		';

		$Consulta2 = "SELECT * FROM `materiassecureprobadas` WHERE materiassecureprobadas.Alumno_idAlumno = 1";
		$Resultado2 = $Mysql->Consulta($Consulta2);
		if(mysqli_num_rows($Resultado2) == 0){
			$html.='<b>b) ¿En tu formación previa reprobaste alguna materia? </b>No <br><br>';
		}else{
			while ($fila2 = $Resultado->fetch_assoc()) {
				$html.='
				<b>b) ¿En tu formación previa reprobaste alguna materia?</b> Si
				¿Cuál? <label for="">'.$fila2['Materias'].'</label>
				Motivo: <label for="">'.$fila2['Motivo'].'</label> <br><br>';
			} }

			$html.='<b>c) ¿Actualmente realizas otro tipo de estudios?</b>
			<label>'.$fila['OtrosEstudios'].'</label><br><br>
			...
			<label >'. $fila['MateriaGustada'].'</label><br><br>
			<b>f) Las asignaturas que más te han interesado en tu formación anterior han sido: </b>
			<label>'. $fila['MateriaDisgustada'].'</label><br><br>
			<b>g) ¿Cómo reaccionan tus padres ante tus calificaciones? </b>
			<label>'.$fila['ReaccionPadres'].'</label><br><br>
			<b>    ¿Cress que cumples con lo que ellos esperan de ti? ¿Por que?</b>
			<label>'.$fila['Espectativa'].'</label> <br><br>
		...
		while ($fila = $Resultado->fetch_assoc()) {
			...
			<label for="">'.$fila['Atraccion'].'</label><br><br>	

			<b>b) Hay algo que te preocupe sobre la nueva etapa que ahora empiezas?</b>
			<label for="">'.$fila['Precupacion'].'</label><br><br>	
			...
			$html.='<t>'.$EstudioEs.'</t><br><br><b>d) Cuando tienes problemas con el estudio ¿a que piensas que se debe?</b>
			<label>'.$fila['ProblemaCausa'].'</label><br><br>	
			...
			$html.='<label>'.$Preparado.'</label><br><br>	
			<b>f) En un profesor lo que más valoras es:</b>
			<label for="">'.$fila['ValorarProfesor'].'</label><br><br>';
		} 
		...
		while ($fila = $Resultado->fetch_assoc()) {
			...
			<b>a) Tiempo de trabajo diario en casa:</b><br><br>	
			<i><b> -Tareas: </b></i>';
			...
			$html.='<br><br>
			<label><i><b> -Tiempo Semanal que le dedicas a la lectura: </b></i></label>';
			...
			$html.='<br><br><b>b) Lugar de Estudio: </b>';
			...
			$html.='<br><br><b>c) Técnicas de estudio que utilizas: </b>';
...
//Close and output PDF document
	$pdf->Output('TutoriaIndividual.pdf', 'I');
